...
public class VirementBean implements Serializable {
	...
	private List<Client> listeClientsAgence = new ArrayList<Client>();
	...
	/**
	 * Méthode qui permet de charger la liste des clients, ainsi que la liste de
	 * leurs comptes bancaires associés, et de les mettre dans une map.
	 */
	public void refresh() {
		...
		int tailleListeDesClients = listeClientsAgence.size();

		for (int i = 0; i < tailleListeDesClients; i++) {
			String prenom = listeClientsAgence.get(i).getPrenom();
			String nom = listeClientsAgence.get(i).getNom();
			long id = listeClientsAgence.get(i).getIdPersonne();
			String prenomEtNom = prenom + " " + nom + ", id : " + String.valueOf(id);
			clients1.put(prenomEtNom, prenomEtNom);
			Map<String, String> map = new HashMap<String, String>();
			listeDesComptes = listeClientsAgence.get(i).getComptes();
	...
	public String validationVirement() {
		...
		if (compte1 != null && client1 != null && compte2 != null && client2 != null && sommeADebiter != null) {
			...
			// try {
			compteDebite = cptService.lireCompteParId((long) idCompte1);
			compteCredite = cptService.lireCompteParId((long) idCompte2);
			...
			// e.printStackTrace();
			// }
			String redirectVrai = menuConseillerBean.navigateBean.redirectToListeClients();
		...
		String redirect = menuConseillerBean.navigateBean
	...
	public void setListeDesClients(List<Client> listeDesClients) {
		this.listeClientsAgence = listeDesClients;
	}
